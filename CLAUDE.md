...
**Claude Code settings:** the global allowlist is system-level managed settings (`/Library/Application Support/ClaudeCode/managed-settings.json`, `/etc/claude-code/` on NixOS) generated by `modules/common/claude-code/default.nix` — edit allowlists there. Only list-valued keys belong in managed settings (scalars like `model` would lock the user out). `~/.claude/settings.json` is Claude-owned; `home/common/claude-code/default.nix` only seeds missing defaults (`env`), and installs the package from `sadjow/claude-code-nix`. Per-repo allowlists go in `.claude/settings.json` committed in that repo. Never use `.claude/settings.local.json` — it's untracked and won't survive a rebuild.
