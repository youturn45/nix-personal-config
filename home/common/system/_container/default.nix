  ...
  programs = {
    k9s = {
      ...
      skins.catppuccin-mocha = let
        skin_file = "${nur-ryan4yin.packages.${pkgs-stable.stdenv.hostPlatform.system}.catppuccin-k9s}/dist/mocha.yml"; # theme - catppuccin mocha
        skin_attr = builtins.fromJSON (
          builtins.readFile
          # replace 'base: &base "#1e1e2e"' with 'base: &base "default"'
          # to make fg/bg color transparent. "default" means transparent in k9s skin.
          (pkgs-stable.runCommandNoCC "get-skin-json" {} ''
            cat ${skin_file} \
              |  sed -E 's@(base: &base ).+@\1 "default"@g' \
              | ${pkgs-stable.yj}/bin/yj > $out
          '')
        );
      in
        skin_attr;
