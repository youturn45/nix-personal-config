    ...
    let
      ...
      transformer =
        ...
        let
          ...
          transformer' =
            defaultDotNixFilePolicy:
            if defaultDotNixFilePolicy == "replace" then
              if hasDefaultDotNixFile then value.default else value
            else if defaultDotNixFilePolicy == "exclude" then
              if hasDefaultDotNixFile then lib.filterAttrs (name: _: name != "default") value else value
            else
              throw "collectModulesRecursively: unknow poilcy for default.nix";
        in
        transformer' (if isRootDir then "exclude" else "replace");
      load =
        src:
        haumeaLib.load {
